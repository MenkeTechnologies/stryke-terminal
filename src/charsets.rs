//! Port of pyte `pyte/charsets.py`.
//!
//! `G0` and `G1` charset mappings, defined the same way as for the linux
//! terminal (see `linux/drivers/tty/consolemap.c`).
//!
//! NOTE (provenance): `VT100_MAP` and `IBMPC_MAP` were taken unchanged from the
//! Linux kernel source and are therefore licensed under **GPL** upstream. They
//! are reproduced here verbatim for faithful emulation parity.
//!
//! Each map is a 256-entry table of Unicode code points: the character at index
//! `i` is the glyph a byte of value `i` renders to under that charset. `LAT1`
//! is the identity map. Translation only rewrites bytes with ordinal `< 256`;
//! higher code points pass through unchanged (matches Python `str.translate`).

/// Latin-1: identity mapping for all 256 byte values.
pub static LAT1_MAP: [u32; 256] = {
    let mut m = [0u32; 256];
    let mut i = 0;
    while i < 256 {
        m[i] = i as u32;
...
/// Look up a charset map by its single-character code (`"B0UV"`), mirroring
/// `pyte.charsets.MAPS`. Returns `None` for unknown codes.
pub fn maps(code: char) -> Option<&'static [u32; 256]> {
    match code {
        'B' => Some(&LAT1_MAP),
        '0' => Some(&VT100_MAP),
        'U' => Some(&IBMPC_MAP),
...
/// Translate a single character through a charset map, matching Python's
/// `str.translate`: bytes with ordinal `< 256` are remapped, everything else
/// passes through unchanged.
#[inline]
pub fn translate(map: &[u32; 256], ch: char) -> char {
    let cp = ch as u32;
    if cp < 256 {
        // All map entries are valid scalar values (verbatim from the kernel
        // tables), so `from_u32` never fails; fall back to the input on the
        // impossible case rather than panicking in the hot draw path.
        char::from_u32(map[cp as usize]).unwrap_or(ch)
    } else {
        ch
    }
}
